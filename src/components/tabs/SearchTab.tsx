...
import {CardUI, SearchBox, SmallCardUI} from '../others/Reusables';
...
const SearchTab = React.memo((props: any) => {
  ...
  const currentSearchMoviePage = useAppSelector(
    state => state.main.searchMoviePage,
  );
  ...
  const handleSearch = React.useCallback(
    async (page = 1) => {
      console.log(
        'currentSearchMoviePage in search:',
      ...
      if (request == undefined)
        return Alert.alert(
          'Error Searching',
          'An error occured while searching please try again',
        );
      console.log(request);
      if (page == 1) {
        setMovieList([]);
  ...
  return (
    <View style={{flex: 1, backgroundColor: theme.appBackground}}>
      <SearchBox onSubmitEditing={handleSearch} />
      <FlashList
        data={movieList}
